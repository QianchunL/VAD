...
# ImageNet normalization (same as original VAD, compatible with DINOv3 LVD models)
img_norm_cfg = dict(
...
bev_w_ = 100
queue_length = 3
total_epochs = 24  # reduced for validation run

# DINOv3 ViT-S/16 embed dim
_dinov3_embed_dim_ = 384

model = dict(
    type='VAD',
    use_grid_mask=False,       # GridMask not suitable for ViT patch embeddings
    ...
    img_neck=dict(
        type='FPN',
        in_channels=[_dinov3_embed_dim_],  # 384 from DINOv3 ViT-S
        out_channels=_dim_,
        start_level=0,
...
train_pipeline = [
    ...
    dict(type='PadMultiViewImage', size_divisor=16),  # patch_size=16 for DINOv3
